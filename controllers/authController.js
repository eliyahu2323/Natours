...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //2)Check of user exits && password is current
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not logged in! Please log in t get access ', 401)
    );
  }
...
exports.isLoggeIn = catchAsync(async (req, res, next) => {
  ...
  if (req.cookies.jwt) {
    ...
    //4) Check if user changed password after the token was issued
    if (correntUser.changePasswordAfter(decoded.iat)) {
      return next();
    }

    //There is a logged in user
    res.locals.user = correntUser;
    return next();
  }
...
exports.restPassword = catchAsync(async (req, res, next) => {
  ...
  //2) If token has not expired and there is uder, set the new password
  if (!user) {
